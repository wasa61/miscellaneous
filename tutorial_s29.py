...
class QLearningController:
    ...
    def update(self, last_state, last_action, reward, next_state):
        if last_state is None or last_action is None:
            return  # Ignore update if no valid previous state
        if last_action >= len(self.actions):
            return  # Prevent invalid action index
...
#                 {"take_width": 2, "market_make_spread": -1, "price_filter_width": 1, "fallback_offset": 1.5, "liquidate_width": 1}
#             ]),
#             "RAINFOREST_RESIN": QLearningController(actions=[
#                 {"take_width": 1, "market_make_spread": -0.5, "price_filter_width": 0.5, "fallback_offset": 2, "liquidate_width": 1},
#             ])
#         }
...
#         if self.strategy_config[symbol]["true_value_args"]["use_fixed"]:
...
class Trader:
    def __init__(self):
        ...
        self.q_controllers = {
            "KELP": QLearningController(actions=[
                ...
                {"take_width": 1, "market_make_spread": -1, "price_filter_width": 1, "fallback_offset": 2, "liquidate_width": 1},
                # {"take_width": 0.5, "market_make_spread": -1, "price_filter_width": 1, "fallback_offset": 2, "liquidate_width": 1},
                # {"take_width": 0.5, "market_make_spread": -1, "price_filter_width": 1, "fallback_offset": 1.5, "liquidate_width": 1},
                # {"take_width": 0.5, "market_make_spread": -1, "price_filter_width": 1, "fallback_offset": 1, "liquidate_width": 1},
                # {"take_width": 1, "market_make_spread": -1, "price_filter_width": 0.5, "fallback_offset": 2, "liquidate_width": 1},
                # {"take_width": 1, "market_make_spread": -1, "price_filter_width": 0.5, "fallback_offset": 1.5, "liquidate_width": 1},
                # {"take_width": 1, "market_make_spread": -1, "price_filter_width": 0.5, "fallback_offset": 1, "liquidate_width": 1},
                # {"take_width": 0.5, "market_make_spread": -1, "price_filter_width": 0.5, "fallback_offset": 2, "liquidate_width": 1},
                # {"take_width": 0.5, "market_make_spread": -1, "price_filter_width": 0.5, "fallback_offset": 1.5, "liquidate_width": 1},
                # {"take_width": 0.5, "market_make_spread": -1, "price_filter_width": 0.5, "fallback_offset": 1, "liquidate_width": 1},
            ]),
            "RAINFOREST_RESIN": QLearningController(actions=[
                {"take_width": 1, "market_make_spread": -1, "price_filter_width": 1, "fallback_offset": 2, "liquidate_width": 1},
            ])
    ...
    def get_state(self, symbol: str, state: TradingState) -> Tuple:
        ...
        # mid_price fallback if no bid/ask
        mid_price = (best_bid + best_ask) / 2 if best_bid and best_ask else true_value
        tv_diff = round(true_value - mid_price)
    ...
    def run(self, state: TradingState) -> Tuple[Dict[str, List[Order]], int, str]:
        ...
        for symbol in self.symbols:
            ...
            if self.print_log:
                print(f"[{symbol}] Action Index Selected: {action_idx}")
