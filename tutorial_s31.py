...
class Trader:
    def __init__(self):
        ...
        self.q_controllers = {
            "KELP": QLearningController(actions=[
                ...
                {"take_width": 2, "market_make_spread": -1, "price_filter_width": 1, "fallback_offset": 1, "liquidate_width": 1},
            ]),
        ...
        self.tv_history = defaultdict(lambda: deque(maxlen=10))
    ...
    def get_state(self, symbol: str, state: TradingState) -> Tuple:
        ...
        true_value = self.strategy_config[symbol]["true_value_args"].get("fixed_value") or get_mid_price(state, symbol)
        mid_price = (best_bid + best_ask) / 2 if best_bid and best_ask else true_value
        tv_diff = round(true_value - mid_price)

        self.tv_history[symbol].append(mid_price)
        vol = compute_volatility(self.tv_history[symbol])
        vol_level = int(vol * 10)  # Bucket volatility

        clipped_position = max(-5, min(5, int(position / 10)))

        return (clipped_position, int(spread), tv_diff, vol_level)
    ...
    def run(self, state: TradingState) -> Tuple[Dict[str, List[Order]], int, str]:
        ...
        for symbol in self.symbols:
            ...
            if self.print_log:
                print(f"[{symbol}] Action Index: {action_idx}, State: {state_tuple}")

            # Assume MarketMakingStrategy is defined externally
            strat = MarketMakingStrategy(
                symbol=symbol,
                limit=self.strategy_config[symbol]["limit"],
                print_log=self.print_log,
                take_order_fn=aggressive_take_orders,
                take_order_args={"take_width": params["take_width"]},
                liquidation_fn=clear_position_liquidation,
                liquidation_args={"liquidate_width": params["liquidate_width"]},
                ...
                true_value_args=self.strategy_config[symbol]["true_value_args"]
            )
            ...
            reward = self.pnl_history[symbol][-1] - self.pnl_history[symbol][0] if len(self.pnl_history[symbol]) >= 15 else 0

            next_state = self.get_state(symbol, state)
            q_ctrl.update(self.last_states[symbol], self.last_actions[symbol], reward, next_state)
    ...
    def save_state(self):
        return json.dumps({
            ...
            "q_tables": {k: self.q_controllers[k].save() for k in self.symbols}
        })
    ...
    def load_state(self, data: str):
        ...
        for k in self.symbols:
            q_data = d.get("q_tables", {}).get(k, {})
            self.q_controllers[k].load(q_data)
